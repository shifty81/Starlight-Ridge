...
impl Default for EditorShellRenderState {
    fn default() -> Self {
        Self {
            ...
            hover_hint: "V Select | B Brush | T Tile Picker | F1/F2/F3 Docks | F5 Reload".to_string(),
...
impl RenderBootstrap {
    pub fn new(
        ...
        let ui_overlay = UiOverlayPipeline::new(&gl)
            .context("failed to build editor shell overlay pipeline")?;
        ...
        let tile_map = match tile_map_data {
            Some(data) => Some(TileMapPipeline::new(&gl, &data).context("failed to build tile map pipeline")?),
            None => None,
        };
        let sprite_sheet = match sprite_data {
            Some(data) => Some(SpritePipeline::new(&gl, &data).context("failed to build sprite pipeline")?),
            None => None,
        };
        let prop_sprite_sheet = match prop_sprite_data {
            Some(data) => Some(SpritePipeline::new(&gl, &data).context("failed to build prop sprite pipeline")?),
            None => None,
        };
    ...
    pub fn replace_tile_map(&mut self, tile_map_data: Option<TileMapRenderData>) -> anyhow::Result<()> {
        ...
        match tile_map_data {
            Some(data) => {
                ...
                self.render_world_height = (data.map_height.max(1) * data.tile_height.max(1)) as f32;
                self.tile_map = Some(
    ...
    pub fn render_frame(
        ...
        if let Some(tile_map) = &self.tile_map {
            ...
            self.grid.draw(&self.gl, self.viewport_width, self.viewport_height);
        ...
        if let Some(lighting) = lighting {
            if lighting.is_visible() {
                self.ui_overlay.draw_screen_tint(&self.gl, [
                    lighting.overlay_r,
                    lighting.overlay_g,
                    lighting.overlay_b,
                    lighting.overlay_a,
        ...
        if self.editor_shell_visible {
            self.ui_overlay.draw_editor_shell(&self.gl, &self.editor_shell_state);
        }
...
impl UiOverlayPipeline {
    ...
    fn draw_editor_shell(&self, gl: &glow::Context, state: &EditorShellRenderState) {
        ...
        let bottom_y1 = if state.bottom_dock_open { -0.835 } else { -0.965 };

        push_ui_rect(&mut vertices, -1.00,  top_y0,  1.00,  1.00, [0.040, 0.046, 0.060, 0.985]);

        if state.left_dock_open {
            push_ui_rect(&mut vertices, -1.00, bottom_y1, left_x1, top_y0, [0.050, 0.058, 0.074, 0.965]);
        ...
        if state.right_dock_open {
            push_ui_rect(&mut vertices, right_x0, bottom_y1, 1.00, top_y0, [0.050, 0.058, 0.074, 0.965]);
        ...
        if state.bottom_dock_open {
            push_ui_rect(&mut vertices, left_x1, -1.00, right_x0, bottom_y1, [0.040, 0.046, 0.060, 0.950]);
            draw_bottom_clean_panel(&mut vertices, state);
        } else {
            push_ui_rect(&mut vertices, -0.360, -0.992, 0.360, -0.960, [0.050, 0.060, 0.078, 0.940]);
            draw_text(&mut vertices, -0.335, -0.972, state.status_message.as_str(), 0.0023, [0.72, 0.82, 0.92, 0.96]);
        }

        // Thin viewport frame only; avoid heavy tinted overlays on top of gameplay.
        push_ui_rect(&mut vertices, left_x1 - 0.003, bottom_y1, left_x1 + 0.003, top_y0, [0.20, 0.27, 0.36, 0.78]);
        push_ui_rect(&mut vertices, right_x0 - 0.003, bottom_y1, right_x0 + 0.003, top_y0, [0.20, 0.27, 0.36, 0.78]);
        push_ui_rect(&mut vertices, left_x1, bottom_y1 - 0.003, right_x0, bottom_y1 + 0.003, [0.20, 0.27, 0.36, 0.78]);
        push_ui_rect(&mut vertices, -1.000, top_y0 - 0.003, 1.000, top_y0 + 0.003, [0.20, 0.27, 0.36, 0.78]);
...
fn push_ui_rect(vertices: &mut Vec<UiVertex>, x0: f32, y0: f32, x1: f32, y1: f32, color: [f32; 4]) {
    ...
    vertices.extend_from_slice(&[
        UiVertex { x: x0, y: y0, r, g, b, a },
        UiVertex { x: x1, y: y0, r, g, b, a },
        UiVertex { x: x1, y: y1, r, g, b, a },
        UiVertex { x: x0, y: y0, r, g, b, a },
        UiVertex { x: x1, y: y1, r, g, b, a },
        UiVertex { x: x0, y: y1, r, g, b, a },
    ]);
}

const TOOL_LABELS: [&str; 10] = [
    "Select", "Pan", "Brush", "Erase", "Fill", "Pick", "Tiles", "Collision", "Assets", "Play",
];

fn draw_top_toolbar(vertices: &mut Vec<UiVertex>, state: &EditorShellRenderState) {
    push_ui_rect(vertices, -0.990, 0.932, -0.944, 0.984, [0.070, 0.083, 0.108, 0.96]);
    draw_text(vertices, -0.978, 0.964, "SR", 0.0032, [0.74, 0.84, 0.96, 0.98]);
    ...
    draw_status_badge(vertices, -0.365, 0.945, TOOL_LABELS.get(state.active_tool).copied().unwrap_or("Tool"));
    ...
    draw_text(vertices, 0.230, 0.965, state.hover_hint.as_str(), 0.0024, [0.72, 0.80, 0.90, 0.95]);
}
...
fn draw_left_clean_panel(vertices: &mut Vec<UiVertex>, state: &EditorShellRenderState) {
    ...
    draw_clean_tab_row(vertices, -0.982, 0.808, &["Project", "Textures", "Maps"], state.active_left_tab);
    draw_clean_asset_row(vertices, -0.972, 0.700, "Terrain Atlas", "active texture atlas", state.selected_asset_index == 0);
    draw_clean_asset_row(vertices, -0.972, 0.570, "Player Walk", "sprite sheet", state.selected_asset_index == 1);
    draw_clean_asset_row(vertices, -0.972, 0.440, "Ocean Props", "static props", state.selected_asset_index == 2);
...
fn draw_right_clean_panel(vertices: &mut Vec<UiVertex>, state: &EditorShellRenderState) {
    ...
    draw_clean_tab_row(vertices, 0.782, 0.808, &["Tile", "Seams", "Export"], state.active_right_tab);
    draw_text(vertices, 0.798, 0.720, "Selected", 0.0031, [0.58, 0.68, 0.80, 0.96]);
    draw_text(vertices, 0.798, 0.675, state.selected_tile_name.as_str(), 0.0032, [0.84, 0.90, 0.98, 0.98]);
    let cell = format!("cell {},{}", state.selected_cell.0, state.selected_cell.1);
    draw_text(vertices, 0.798, 0.632, cell.as_str(), 0.0028, [0.68, 0.76, 0.86, 0.96]);
    let role = format!("role {}", state.selected_role);
    draw_text(vertices, 0.798, 0.592, role.as_str(), 0.0025, [0.68, 0.80, 0.70, 0.96]);
    let collision = format!("collision {}", state.selected_collision);
    draw_text(vertices, 0.798, 0.555, collision.as_str(), 0.0025, [0.78, 0.70, 0.58, 0.96]);
...
fn draw_bottom_clean_panel(vertices: &mut Vec<UiVertex>, state: &EditorShellRenderState) {
    draw_clean_tab_row(vertices, -0.720, -0.872, &["Console", "Validation", "Hot Reload", "Runtime"], state.active_bottom_tab);
    draw_text(vertices, -0.705, -0.930, state.status_message.as_str(), 0.0029, [0.76, 0.86, 0.96, 0.98]);
    draw_text(vertices, -0.705, -0.970, "Click toolbar icons or use shortcuts: V B E G I T C A P. F5 reloads content.", 0.0025, [0.58, 0.68, 0.80, 0.96]);
}

fn draw_collapsed_dock_tab(vertices: &mut Vec<UiVertex>, x: f32, y: f32, label: &str) {
    push_ui_rect(vertices, x, y, x + 0.090, y + 0.040, [0.070, 0.085, 0.112, 0.96]);
    draw_text(vertices, x + 0.010, y + 0.026, label, 0.0024, [0.76, 0.86, 0.96, 0.96]);
}

fn draw_clean_panel_title(vertices: &mut Vec<UiVertex>, x: f32, y: f32, title: &str, hint: &str) {
    push_ui_rect(vertices, x, y, x + 0.195, y + 0.052, [0.080, 0.098, 0.128, 0.98]);
    push_ui_rect(vertices, x, y, x + 0.008, y + 0.052, [0.40, 0.58, 0.78, 0.98]);
    draw_text(vertices, x + 0.016, y + 0.034, title, 0.0032, [0.84, 0.90, 0.98, 0.98]);
    draw_text(vertices, x + 0.105, y + 0.030, hint, 0.0021, [0.56, 0.66, 0.78, 0.94]);
}

fn draw_clean_tab_row(vertices: &mut Vec<UiVertex>, x: f32, y: f32, labels: &[&str], active_index: usize) {
    let mut tx = x;
    for (index, label) in labels.iter().enumerate() {
        let w = ((*label).len() as f32 * 0.014).clamp(0.070, 0.130);
        let color = if index == active_index { [0.145, 0.180, 0.235, 0.98] } else { [0.075, 0.088, 0.112, 0.94] };
        push_ui_rect(vertices, tx, y, tx + w, y + 0.045, color);
        draw_text(vertices, tx + 0.009, y + 0.029, label, 0.0024, [0.78, 0.86, 0.96, 0.98]);
        tx += w + 0.008;
    }
}

fn draw_clean_asset_row(vertices: &mut Vec<UiVertex>, x: f32, y: f32, title: &str, sub: &str, active: bool) {
    let border = if active { [0.42, 0.61, 0.82, 0.95] } else { [0.18, 0.23, 0.30, 0.90] };
    push_ui_rect(vertices, x, y, x + 0.210, y + 0.092, border);
    push_ui_rect(vertices, x + 0.004, y + 0.004, x + 0.206, y + 0.088, [0.066, 0.078, 0.102, 0.98]);
    push_ui_rect(vertices, x + 0.014, y + 0.020, x + 0.060, y + 0.072, [0.40, 0.58, 0.19, 0.98]);
    draw_text(vertices, x + 0.074, y + 0.061, title, 0.0027, [0.84, 0.90, 0.98, 0.98]);
    draw_text(vertices, x + 0.074, y + 0.031, sub, 0.0022, [0.58, 0.68, 0.80, 0.94]);
}

fn draw_step_chip(vertices: &mut Vec<UiVertex>, x: f32, y: f32, label: &str) {
    push_ui_rect(vertices, x, y, x + 0.160, y + 0.040, [0.068, 0.082, 0.106, 0.94]);
    draw_text(vertices, x + 0.010, y + 0.026, label, 0.0024, [0.70, 0.80, 0.90, 0.96]);
}

fn draw_clean_preview(vertices: &mut Vec<UiVertex>, x: f32, y: f32, title: &str, mode: usize) {
    push_ui_rect(vertices, x, y, x + 0.078, y + 0.145, [0.080, 0.095, 0.124, 0.96]);
    draw_text(vertices, x + 0.009, y + 0.126, title, 0.0024, [0.78, 0.86, 0.96, 0.96]);
    match mode {
        0 => draw_mini_tile(vertices, x + 0.020, y + 0.034, 0.042, 0.052, [0.42, 0.58, 0.16, 0.98]),
...
fn draw_mini_tile(vertices: &mut Vec<UiVertex>, x: f32, y: f32, w: f32, h: f32, color: [f32; 4]) {
    ...
    push_ui_rect(vertices, x, y + h - 0.004, x + w, y + h, [0.24, 0.38, 0.10, 0.90]);
    push_ui_rect(vertices, x, y, x + 0.004, y + h, [0.70, 0.78, 0.44, 0.90]);
    push_ui_rect(vertices, x + w - 0.004, y, x + w, y + h, [0.24, 0.38, 0.10, 0.90]);
}
...
fn draw_seam_grid(vertices: &mut Vec<UiVertex>, x: f32, y: f32) {
    ...
    for row in 0..3 {
        for col in 0..3 {
            ...
            draw_mini_tile(vertices, x + col as f32 * s, y + row as f32 * s, s, s, shade);
        }
    }
    push_ui_rect(vertices, x + s - 0.0015, y, x + s + 0.0015, y + s * 3.0, [0.85, 0.40, 0.30, 0.90]);
    push_ui_rect(vertices, x + s * 2.0 - 0.0015, y, x + s * 2.0 + 0.0015, y + s * 3.0, [0.85, 0.40, 0.30, 0.90]);
}
...
fn draw_atlas_mini(vertices: &mut Vec<UiVertex>, x: f32, y: f32) {
    ...
    push_ui_rect(vertices, x, y, x + w, y + h * 0.30, [0.80, 0.62, 0.34, 0.98]);
    push_ui_rect(vertices, x, y, x + w, y + h * 0.13, [0.10, 0.50, 0.70, 0.98]);
    push_ui_rect(vertices, x + 0.004, y + h - 0.018, x + 0.020, y + h - 0.002, [0.75, 0.88, 0.98, 0.98]);
}

fn draw_icon_action(vertices: &mut Vec<UiVertex>, x: f32, y: f32, title: &str) {
    push_ui_rect(vertices, x, y, x + 0.064, y + 0.045, [0.095, 0.120, 0.158, 0.96]);
    push_ui_rect(vertices, x + 0.010, y + 0.014, x + 0.024, y + 0.030, [0.44, 0.62, 0.82, 0.96]);
    draw_text(vertices, x + 0.030, y + 0.029, title, 0.0021, [0.80, 0.88, 0.96, 0.96]);
}

fn draw_status_badge(vertices: &mut Vec<UiVertex>, x: f32, y: f32, title: &str) {
    let w = (title.len() as f32 * 0.014).clamp(0.080, 0.140);
    push_ui_rect(vertices, x, y, x + w, y + 0.034, [0.070, 0.092, 0.120, 0.94]);
    draw_text(vertices, x + 0.008, y + 0.023, title, 0.0022, [0.70, 0.82, 0.94, 0.95]);
}

fn draw_toolbar_icon_state(vertices: &mut Vec<UiVertex>, x: f32, y: f32, index: usize, active: bool) {
    let button = if active { [0.180, 0.230, 0.305, 0.98] } else { [0.085, 0.100, 0.130, 0.98] };
    let mark = [0.72, 0.78, 0.86, 0.98];
    let accent = if active { [0.55, 0.76, 0.98, 0.98] } else { [0.42, 0.58, 0.76, 0.98] };
    let w = 0.042;
    let h = 0.046;
    push_ui_rect(vertices, x, y, x + w, y + h, button);
    if active { push_ui_rect(vertices, x, y, x + w, y + 0.004, accent); }
    let cx = x + 0.021; let cy = y + 0.023;
    match index {
        0 => { push_ui_rect(vertices, cx - 0.010, cy - 0.012, cx - 0.004, cy + 0.014, mark); push_ui_rect(vertices, cx - 0.004, cy - 0.003, cx + 0.012, cy + 0.004, mark); }
        1 => { push_ui_rect(vertices, cx - 0.012, cy - 0.012, cx + 0.008, cy + 0.000, mark); push_ui_rect(vertices, cx + 0.006, cy - 0.002, cx + 0.014, cy + 0.014, accent); }
        2 => push_ui_rect(vertices, cx - 0.013, cy - 0.010, cx + 0.013, cy + 0.010, mark),
        3 => { push_ui_rect(vertices, cx - 0.012, cy - 0.003, cx + 0.012, cy + 0.012, mark); push_ui_rect(vertices, cx - 0.005, cy - 0.014, cx + 0.005, cy - 0.005, accent); }
        4 => { push_ui_rect(vertices, cx - 0.012, cy - 0.012, cx - 0.004, cy + 0.012, mark); push_ui_rect(vertices, cx - 0.004, cy + 0.005, cx + 0.014, cy + 0.012, accent); }
        5 => { push_ui_rect(vertices, cx - 0.014, cy - 0.014, cx + 0.014, cy - 0.010, mark); push_ui_rect(vertices, cx - 0.014, cy + 0.010, cx + 0.014, cy + 0.014, mark); push_ui_rect(vertices, cx - 0.014, cy - 0.014, cx - 0.010, cy + 0.014, mark); push_ui_rect(vertices, cx + 0.010, cy - 0.014, cx + 0.014, cy + 0.014, mark); }
        6 => { push_ui_rect(vertices, cx - 0.003, cy - 0.014, cx + 0.003, cy + 0.014, mark); push_ui_rect(vertices, cx - 0.014, cy - 0.003, cx + 0.014, cy + 0.003, mark); }
        7 => { push_ui_rect(vertices, cx - 0.014, cy - 0.014, cx - 0.001, cy + 0.014, accent); push_ui_rect(vertices, cx + 0.002, cy - 0.014, cx + 0.014, cy + 0.014, mark); }
        8 => { push_ui_rect(vertices, cx - 0.014, cy - 0.011, cx - 0.005, cy + 0.011, mark); push_ui_rect(vertices, cx - 0.001, cy - 0.011, cx + 0.009, cy + 0.011, accent); push_ui_rect(vertices, cx + 0.012, cy - 0.011, cx + 0.015, cy + 0.011, mark); }
        _ => { push_ui_rect(vertices, cx - 0.006, cy + 0.004, cx + 0.006, cy + 0.016, mark); push_ui_rect(vertices, cx - 0.012, cy - 0.016, cx + 0.012, cy + 0.002, accent); }
    }
}


fn draw_text(vertices: &mut Vec<UiVertex>, x: f32, y: f32, text: &str, scale: f32, color: [f32; 4]) {
    let mut cursor = x;
        ...
        for (row, bits) in glyph.iter().enumerate() {
            for col in 0..5 {
                if (*bits & (1 << (4 - col))) != 0 {
                    ...
                    push_ui_rect(vertices, x0, y0 - scale, x0 + scale * 0.82, y0 - scale * 0.18, color);
                }
            }
        }
        cursor += scale * 6.0;
...
impl TileMapPipeline {
    fn new(gl: &glow::Context, data: &TileMapRenderData) -> anyhow::Result<Self> {
        ...
        let texture = create_texture_from_file(gl, &data.texture_path)
            .with_context(|| format!("failed to load tile atlas {}", data.texture_path.display()))?;
    ...
    fn draw(&self, gl: &glow::Context, viewport_width: u32, viewport_height: u32) {
        unsafe {
            ...
            if let Some(location) = gl.get_uniform_location(self.program, "u_viewport_size") {
                gl.uniform_2_f32(Some(&location), viewport_width.max(1) as f32, viewport_height.max(1) as f32);
            }
            if let Some(location) = gl.get_uniform_location(self.program, "u_world_size") {
                gl.uniform_2_f32(Some(&location), self.world_width.max(1.0), self.world_height.max(1.0));
            }
...
fn build_tile_vertices(data: &TileMapRenderData, world_width: f32, world_height: f32) -> Vec<TileVertex> {
    ...
    for tile in &data.tiles {
        ...
        vertices.extend_from_slice(&[
            TileVertex { x: x0, y: y0, u: u0, v: v_top },
            TileVertex { x: x1, y: y0, u: u1, v: v_top },
            TileVertex { x: x1, y: y1, u: u1, v: v_bottom },
            TileVertex { x: x0, y: y0, u: u0, v: v_top },
            TileVertex { x: x1, y: y1, u: u1, v: v_bottom },
            TileVertex { x: x0, y: y1, u: u0, v: v_bottom },
        ]);
    }
...
fn create_texture_from_file(gl: &glow::Context, path: &std::path::Path) -> anyhow::Result<glow::Texture> {
...
impl SpritePipeline {
    fn new(gl: &glow::Context, data: &SpriteRenderData) -> anyhow::Result<Self> {
        ...
        let vbo = unsafe { gl.create_buffer() }
            .map_err(|e| anyhow::anyhow!("create sprite VBO: {e}"))?;
        let texture = create_texture_from_file(gl, &data.texture_path)
            .with_context(|| format!("failed to load sprite sheet {}", data.texture_path.display()))?;
...
impl TexturedQuadPipeline {
    fn new(gl: &glow::Context) -> anyhow::Result<Self> {
        let program = create_program(gl, TEXTURED_QUAD_VERTEX_SHADER, TEXTURED_QUAD_FRAGMENT_SHADER)
        ...
        Ok(Self { program, vao, vbo, ebo, texture })
    }
...
impl GridPipeline {
    fn new(gl: &glow::Context) -> anyhow::Result<Self> {
        ...
        let vbo = unsafe { gl.create_buffer() }
            .map_err(|e| anyhow::anyhow!("create grid VBO: {e}"))?;
        ...
        Ok(Self { program, vao, vbo, vertex_count: (vertices.len() / 2) as i32 })
    }

    fn draw(&self, gl: &glow::Context, viewport_width: u32, viewport_height: u32) {
        let aspect = if viewport_height == 0 { 1.0 } else { viewport_width as f32 / viewport_height as f32 };
...
fn create_program(gl: &glow::Context, vertex_source: &str, fragment_source: &str) -> anyhow::Result<glow::Program> {
    unsafe {
        let program = gl.create_program().map_err(|e| anyhow::anyhow!("create GL program: {e}"))?;
        let shaders = [(glow::VERTEX_SHADER, vertex_source), (glow::FRAGMENT_SHADER, fragment_source)];

        let mut compiled = Vec::with_capacity(shaders.len());
        for (kind, source) in shaders {
            let shader = gl.create_shader(kind).map_err(|e| anyhow::anyhow!("create GL shader: {e}"))?;
