...
fn main() {
    ...
    let listener = match TcpListener::bind(&bind_addr) {
        ...
        Err(error) => {
            ...
            eprintln!(
                "Try a different port: STARLIGHT_WEB_PORT=8788 cargo run -p web_editor_server"
            );
            std::process::exit(1);
        }
    };
...
fn handle_connection(
    ...
    match (method, path) {
        ...
        ("GET", "/api/server_info") => {
            serve_server_info(stream, root, write_enabled, &request_host)
        }
        ("GET", "/api/manifest") => serve_manifest(stream, root, write_enabled, &request_host),
        _ if method == "GET" && path.starts_with("/api/map/") => {
            serve_map_bundle(stream, root, &path[9..])
        }
        _ if method == "POST" && path.starts_with("/api/save/map/") => {
            save_map_layers(stream, root, &path[14..], &body, write_enabled)
        }
        _ if method == "GET" && path.starts_with("/web/") => {
            let rel = &path[5..];
            serve_safe_file(stream, &root.join("tools/web_editor"), rel)
        }
        _ if method == "GET" && path.starts_with("/assets/") => {
            serve_safe_file(stream, root, &path[1..])
        }
        _ if method == "GET" && path.starts_with("/content/") => {
            serve_safe_file(stream, root, &path[1..])
        }
...
fn save_map_layers(
    ...
    if !is_safe_segment(map_id) || file_name != "layers.ron" {
        return write_response(
            ...
            400,
            "text/plain; charset=utf-8",
            b"Only map layers.ron can be saved from this early web editor\n",
        );
    }
    ...
    if !text.contains("layers:") || !text.contains("rows:") {
        return write_response(
            ...
            400,
            "text/plain; charset=utf-8",
            b"layers.ron validation failed: missing layers/rows\n",
        );
    }
...
fn tileset_file_for(map_ron: &str) -> String {
    let tileset_id =
        extract_quoted_after(map_ron, "tileset:").unwrap_or_else(|| "base_tiles".to_string());
